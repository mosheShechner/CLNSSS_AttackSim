...
class Attack_round_dependency:
    def __init__(self, nof_repititions = 20, sd_list = [1.0], nof_sim_keys=3, tail_size=1000, b=30, l_list=[100, 200, 300, 400, 500, 600, 700, 800, 900, 1000], seed=None):
    ...
    def record_attack_round_dependency(self, file_pref = "sim_2"):
        ...
        for l_idx in range(len(self.l_list)):
            ...
            file_name = "./results/{0}_ell_{1}.csv".format(file_pref, self.l_list[l_idx])
    ...
    def simulate_median_attack(self):
        ...
        while abs(key_0_bias) < nof_sd or abs(key_1_bias) < nof_sd:
            ...
            win_round += 1
        print("For l = {3}, sd = {4}, attack wins after {2} rounds. Bias: key 0 = {0}, key 1 = {1}.".format(key_0_bias, key_1_bias, win_round, self.l, nof_sd))
        return win_round
...
# test 1:  5 repititions of SD = 4
test = Attack_round_dependency(nof_repititions = 5, sd_list = [4])
print(test)
test.record_attack_round_dependency(file_pref = "sim_2_sd_4")

# test 2: 20 repititions of SD = 1
test = Attack_round_dependency(nof_repititions = 20, sd_list = [1])
print(test)
